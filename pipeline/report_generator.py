...
docs/ is what GitHub Pages serves, and everything it loads lives inside it --
no CDN, no external fonts. `--vendor` fetches those assets once; after that the
...
def copy_artifacts(runs: List[Dict[str, Any]], docs_dir: Path) -> None:
    """Copy each run's bundle into docs/data/<slug>/, replacing what was there."""
    for meta in runs:
        target = docs_dir / "data" / meta["slug"]
...
# --- Vendored assets ----------------------------------------------------------
#
# The page must not reach the network when someone opens it, so everything it
# loads is committed under docs/. These are fetched on demand rather than at
# build time -- a build should not need the internet.

VENDOR_ASSETS = {
    "js/vendor/d3.min.js": "https://cdn.jsdelivr.net/npm/d3@7.9.0/dist/d3.min.js",
...
def vendor_assets(docs_dir: Path = DOCS_DIR) -> None:
    """
    Download the third-party assets the page needs into docs/, once.

    Fonts take an extra step: Google serves a stylesheet of @font-face rules
    pointing at its own CDN, so the woff2 files behind it are pulled down and the
    URLs rewritten to local paths.
    """
    import requests

    for rel, url in VENDOR_ASSETS.items():
        target = docs_dir / rel
        target.parent.mkdir(parents=True, exist_ok=True)
        response = requests.get(url, timeout=30)
        response.raise_for_status()
        target.write_bytes(response.content)
        print(f"[report_generator] vendored {rel} ({len(response.content) / 1024:.0f} KB)")
...
def main() -> None:
    ...
    parser.add_argument(
        ...
        help="Download third-party assets into docs/ (needs network; run once, then commit).",
    )
